package com.rbgt.ddd.domain.entity;
...
import com.rbgt.ddd.domain.valobj.Address;
import com.rbgt.ddd.domain.valobj.Contract;
import com.rbgt.ddd.domain.valobj.OrderStatus;
import com.rbgt.ddd.domain.valobj.Person;
 ...
 * 2.注意看实体和值对象的lombok注解之间的区别，值对象多了@@EqualsAndHashCode. 因为实体通过基类的id来确定唯一标识，而值对象是全字段来确定唯一性。 3.构建对象有俩种方式： 3.1
 * 字段少、简单的实体，可以直接在实体内，创建create来构建实体对象,入参只需初始化就有的字段，比如payTime、finishTime初始化就是null不需要作为入参。 3.2
 * 字段多，复杂的实体，可以新建Factory类，来构建实体对象。 4.DDD推荐把实体的所有基础类型字段尽可能的建模成值对象(例：String orderNo -> OrderNo orderNo, String id ->
 * OrderId id)
 *
 * @author lvweijie
...
public class Order {
     ...
     */
    public static Order create(String orderNo, OrderStatus status, Address collectAddress, Person collectPerson,
        Contract contract, Set<OrderItem> orderItems) {
        return Order.builder().orderNo(orderNo).status(status).collectAddress(collectAddress)
            .collectPerson(collectPerson).contract(contract).orderItems(orderItems).build();
    }
